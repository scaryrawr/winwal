...
$commonPath = Join-Path -Path $srcDir -ChildPath "Common/WalTheme.psm1"

if (Test-Path -Path $commonPath) {
    Import-Module -Name $commonPath -Force
} else {
    Write-Warning "Could not find new module structure at $commonPath. Falling back to legacy implementation."
